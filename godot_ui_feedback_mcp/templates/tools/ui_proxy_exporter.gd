...
static func render_scene_html(root: Node, viewport_size: Vector2, options: Dictionary = {}) -> String:
	var title := str(options.get("title", "Godot UI Capture"))
...
static func _is_content_control(control: Control) -> bool:
	if control is BaseButton or control is Label or control is RichTextLabel:
		return true
	if control is Range or control is LineEdit or control is TextEdit:
		return true
	if control.get_class() in ["OptionButton", "TabBar", "ItemList", "Tree"]:
		return true
	if control.mouse_filter != Control.MOUSE_FILTER_IGNORE and control.focus_mode != Control.FOCUS_NONE:
		return true
