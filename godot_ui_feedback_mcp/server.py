...
def call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name not in registry:
        raise core.UiFeedbackMcpError(f"Unknown UI feedback MCP tool: {name}")
...
def _handle_capture_godot_ui_reference(arguments: dict[str, Any]) -> dict[str, Any]:
    return core.capture_godot_ui_reference(
		...
		godot_bin=str(arguments.get("godot_bin", "godot")),
		calls=list(arguments.get("calls", [])),
...
def _required(arguments: dict[str, Any], key: str) -> Any:
    if key not in arguments or arguments[key] in ("", None):
        raise core.UiFeedbackMcpError(f"Missing required argument: {key}")
...
def _run_mcp_server() -> int:
    ...
    def ensure_exporter_installed(project_path: str) -> dict[str, Any]:
        """Install the Godot proxy exporter scripts into the target Godot project."""
...
def _run_minimal_stdio_server() -> int:
    for line in sys.stdin:
        ...
        if not line:
            continue
        response = dispatch_json_rpc(json.loads(line))
    ...
    return [
        {
            ...
            "inputSchema": {
                ...
                "properties": {
					...
					"godot_bin": {"type": "string", "default": "godot"},
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "width": {"type": "integer", "default": 1280},
                    "height": {"type": "integer", "default": 720},
					"title": {"type": "string", "default": "Godot UI Proxy"},
					"godot_bin": {"type": "string", "default": "godot"},
					"calls": {"type": "array", "items": {"type": "string"}, "default": []},
				},
                "required": ["project_path", "scene_path", "out_path"],
        ...
        },
        {
            ...
            "description": "Install the Godot proxy exporter scripts into the target project.",
            "inputSchema": {
                "type": "object",
